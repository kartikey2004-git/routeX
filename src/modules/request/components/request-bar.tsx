  ...
  return (
    <div className="flex items-center justify-between bg-card border px-3 py-2 w-full shadow-sm">
        ...
        >
          <SelectTrigger
            className={`w-28 font-semibold text-sm focus:ring-0 border-none shadow-none ${
              requestColorMap[tab.method] || "text-slate-400"
            }`}
          >
            <SelectValue />
          </SelectTrigger>
      ...
      <Button
        ...
        className="ml-3 flex items-center gap-2 bg-black hover:bg-black/90 disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-semibold px-4 py-2 rounded-sm transition-all duration-200 shadow-md"
      >
        {isPending ? (
          <>
